...
- 所有用户可见的后台读取、目录扫描、导入、轨迹/视频帧载入和结果填充都会显示进度条：文件数量可知时显示完成数和百分比，无法预估总量时显示持续进行状态；完成、失败、取消或切换项目后会自动隐藏，避免留下过期进度。
...
- “二维质检”不会在打开项目时自动扫描。进入页面后点击“开始二维质检”，程序会在后台逐帧读取 `pose/<camera>_json/*.json`（或项目内合并 pose JSON），并显示已读取文件数和百分比；以默认阈值 0.500 找出低置信度和缺失关节点。可按相机、帧范围、二维姿态数据中实际出现的人物原始序号（从人物 0 开始）、可勾选的多个关键点和置信度（≤ 或 ≥ 自定义阈值）组合条件；调整条件后点击“开始筛选”，才会同时刷新二维质检表和二维修正的问题队列。结果每页最多显示 200 项，避免大量问题使界面卡顿；每项显示相机、原始帧、人物序号、关节点和实际置信度。点击条目可直接打开同一原始帧进行修正，不依赖同步或多人关联结果。首次保存保留备份，撤销、重做和文件恢复同时处理坐标与置信度。
- “二维修正”左侧问题队列会显示当前二维质检报告中的问题，可按处理状态筛选并分页浏览；点击问题会打开对应相机、原始帧、人物和关节点。打开已有项目时还会枚举 `pose/<camera>_json`，可直接切换相机、已有帧、人物和关节点。1/2/4 路模式分别使用单画面、左右双画面和 2×2 四画面；统一时间轴会通过同步映射刷新所有可见相机，缺少同步映射时明确按同一原始帧浏览。视频来源按“用户指定优先项 → 原视频 → Pose2Sim 二维标记视频”选择，并在画布标题中明确标注。滚轮以鼠标位置为中心缩放，左键拖动画布空白处或中键可平移，拖动选中关节点仍用于修正。骨架左侧为蓝色、右侧为橙色、中轴为青绿色；未知模型只显示明确的索引点，不猜测骨架关系。
