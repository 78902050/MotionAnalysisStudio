...
- “二维质检”不会在打开项目时自动扫描。进入页面后点击“开始二维质检”，程序会在后台逐帧读取 `pose/<camera>_json/*.json`（或项目内合并 pose JSON），并显示已读取文件数和百分比；以默认阈值 0.500 找出低置信度和缺失关节点。可按相机、帧范围、关键点和置信度（≤ 或 ≥ 自定义阈值）组合筛选。结果每页最多显示 200 项，避免大量问题使界面卡顿；每项显示相机、原始帧、人物序号、关节点和实际置信度。点击条目可直接打开同一原始帧进行修正，不依赖同步或多人关联结果。首次保存保留备份，撤销、重做和文件恢复同时处理坐标与置信度。
