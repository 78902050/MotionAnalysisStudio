...
- “媒体”在后台扫描视频元数据，列表较大时不会在界面线程逐个读取。可为表格中选中的相机导入任意文件名的原视频，也可选择一个文件夹，按 `cam01.mp4`、`cam01_*.mp4` 或 `cam01-*.mp4` 形式为多台相机批量绑定；多个候选不会自动猜测。每台相机可分别绑定原视频和 Pose2Sim 二维标记视频并设置优先来源；绑定只更新项目清单，不复制、不修改或转码视频。
