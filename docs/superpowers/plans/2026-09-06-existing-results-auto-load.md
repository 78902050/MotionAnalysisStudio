...
**Architecture:** 以增强后的 `ExistingResultDiscovery` 作为目录事实来源，登记器刷新清单，页面通过明确的适配接口读取标准 Pose2Sim/Caliscope 产物。没有新增只做转发的 `ProjectArtifactCatalog`；现有发现器已承担同一职责，继续复用可避免两套目录规则漂移。耗时解析继续放在后台任务中，页面之间只传递经过验证的领域对象。
...
**Interfaces:**
- Produces: `ExistingResultDiscovery.discover_one(root: Path) -> TrialCandidate`
- Produces: `ExistingResultImporter.register(candidate: TrialCandidate) -> ProjectManager`

- [x] 写失败测试，构造已有清单后再加入标准结果目录并重新登记：
...
- [x] 运行 `\.venv\Scripts\python.exe -m unittest tests.test_existing_result_refresh -v`，确认相机仍为空或缺少 `imported_artifacts`。
- [x] 复用 `ExistingResultDiscovery` 集中目录枚举，并让 `ExistingResultImporter.register()` 对已有清单执行刷新而不是提前返回。
- [x] 运行 `\.venv\Scripts\python.exe -m unittest tests.test_existing_result_refresh tests.test_project_manager tests.test_existing_results_import -v`，全部通过。
- [x] 仅暂存本任务文件并提交 `fix: refresh imported result manifests`。
...
- [x] 运行 `\.venv\Scripts\python.exe -m unittest tests.test_existing_result_collection_import -v`，确认入口返回失败且候选数为 0。
- [x] 在 `import_existing_path()` 捕获“目录不是单试次”后调用既有 `scan_existing_parent()`；扫描完成时区分 0、1、多候选并显示可执行提示。
- [x] 运行 `\.venv\Scripts\python.exe -m unittest tests.test_existing_result_collection_import tests.test_existing_results_gui -v`，全部通过且 Qt 线程退出。
- [x] 仅暂存本任务文件并提交 `fix: expand processed result collections`。
...
- [x] 写失败测试，直接解析标准 Pose2Sim 帧并创建可编辑会话：
...
- [x] 写 GUI 失败测试：`CorrectionPage` 的人物和关节点选择器可用，改变相机/帧发出 `browse_requested(camera, frame, person, keypoint)`。
- [x] 运行 `\.venv\Scripts\python.exe -m unittest tests.test_existing_pose_browser -v`，确认因 `resolve_pose_frame` 和 `browse_requested` 不存在而失败。
- [x] 增加 pose 帧索引与 `HALPE_26` 名称表；实现直接解析、选择器更新和 `CorrectionSession` 复用。无法识别模型时生成 `index-000` 形式名称并在状态栏说明。
- [x] 运行 `\.venv\Scripts\python.exe -m unittest tests.test_existing_pose_browser tests.test_correction_workspace tests.test_correction_session tests.test_correction_history -v`，全部通过。
- [x] 仅暂存本任务文件并提交 `feat: browse imported pose frames directly`。
...
**Interfaces:**
- Consumes: `ExistingResultDiscovery` 的标准目录清单。
- Produces: TRC 三维质量指标和项目打开后的只读关联结果摘要。

- [x] 写失败测试，使用最小 TRC 和标准嵌套 pose：
...
- [x] 写 GUI 失败测试：打开无报告的已有结果项目会启动质量任务；打开含 `pose-associated` 的项目会显示相机、文件数和帧范围。
- [x] 运行 `\.venv\Scripts\python.exe -m unittest tests.test_existing_quality_and_association -v`，确认 TRC 指标缺失且关联结果没有摘要。
- [x] 用 `Trajectory.from_trc()` 生成三维质量汇总；项目打开时按项目 ID/generation 启动可取消的质量任务。关联结果只展示已有文件证据，不在缺少映射时推断人物身份。
- [x] 运行 `\.venv\Scripts\python.exe -m unittest tests.test_existing_quality_and_association tests.test_quality_audit tests.test_quality_pages tests.test_association -v`，全部通过。
- [x] 仅暂存本任务文件并提交 `feat: load imported quality and associations`。
...
- [x] 运行 `\.venv\Scripts\python.exe -m unittest tests.test_existing_analysis_flow -v`，确认产物控件或 `metrics_ready` 信号不存在。
- [x] 增加 TRC 选择器和 MOT/STO 只读目录；`AnalysisPage` 完成计算后发出 `MetricTable`，主窗口调用 `EventsPage.set_metric_table()` 并创建当前试次 `ComparisonMember`。
- [x] 运行 `\.venv\Scripts\python.exe -m unittest tests.test_existing_analysis_flow tests.test_analysis_page tests.test_events_page tests.test_comparison_page -v`，全部通过。
- [x] 仅暂存本任务文件并提交 `feat: connect imported analysis results`。
...
- [x] 扩展 `tests/test_existing_results_acceptance.py`，断言集合候选、二维直接浏览、TRC 质量和页面装载均成功；旧清单刷新与指标联动由各自聚焦测试覆盖。
- [x] 运行 `\.venv\Scripts\python.exe -m unittest tests.test_existing_results_acceptance -v`，真实格式适配通过。
- [x] 运行 `powershell -ExecutionPolicy Bypass -File scripts\run_real_data_acceptance.ps1 -Root D:\test\data`，副本验收通过，源 Pose 文件内容不变，TRC/MOT/STO 未写入。
- [x] 运行 `\.venv\Scripts\python.exe -m unittest discover -s tests -q` 和 `\.venv\Scripts\python.exe -m compileall -q app tests scripts`，253 项测试和编译全部通过。
- [x] 运行 `powershell -ExecutionPolicy Bypass -File scripts\build_windows.ps1`，再运行 `powershell -ExecutionPolicy Bypass -File scripts\smoke_exe.ps1 -Executable outputs\build\dist\MotionAnalysisStudio.exe -Mode All`，三类 smoke 全部通过。
- [x] 更新用户指南和测试记录，只暂存本任务文件并提交 `test: verify imported result auto loading`。
